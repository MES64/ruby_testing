  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      ...
      it 'completes loop and displays error message once' do
        ...
        max = game_input.instance_variable_get(:@maximum)
        error_message = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input).to receive(:puts).exactly(1).time.with(error_message)
        game_input.player_input(min, max)
      end
    end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        invalid_number = '99'
        valid_input = '5'
        allow(game_input).to receive(:gets).and_return(invalid_letter, invalid_number, valid_input)
      end
      ...
      it 'completes loop and displays error message twice' do
        ...
        max = game_input.instance_variable_get(:@maximum)
        error_message = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input).to receive(:puts).exactly(2).times.with(error_message)
        game_input.player_input(min, max)
      end
    end
  end
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = game_verify.instance_variable_get(:@maximum)
        invalid_input = -6
        denied_input = game_verify.verify_input(min, max, invalid_input)
        expect(denied_input).to be_nil
      end
    end
  end
